   ...
   <div class="kt-portlet__head-toolbar">
      <div class="kt-portlet__head-wrapper">
         <div class="kt-portlet__head-actions">
            ...
            &nbsp;
            <router-link to="/admin/newopportunity" class="btn btn-brand btn-elevate btn-icon-sm">
    ...
    <thead>
        <tr>
            ...
            <th class="" style="text-align: center; cursor: pointer;"><span> Amount </span><span>↕</span></th>
            <th class="" style="text-align: center; cursor: pointer;"><span> Sale Stage </span><span>↕</span></th>
    ...
    <tbody>
        <tr v-for="val in rows">
          <td>{{val.name}}</td>
          <td>{{val.amount}}</td>
          <td>{{val.sale_stage}}</td>
          <td>{{val.status}}</td>
             <td>
             <router-link :to="'/admin/opportunity/'+val.id" class="btn btn-sm btn-clean btn-icon btn-icon-md">   <i class="la la-edit"></i>
             </router-link> 
             <a title="Delete" class="btn btn-sm btn-clean btn-icon btn-icon-md" @click="removeOpp(val.id)"><i class="la la-trash"></i></a>
            </td>
        </tr>
    </tbody>
    <!---->
...
<script type="text/javascript">
    ...
    components: {
      'app-notify': notification
		...
		methods : {
			getList () {
				Opportunity.list({page:this.limit}).then(response=>{
          ...
          .then(function () {
             Opportunity.remove(id).then(response=>{
                   console.log(this)
